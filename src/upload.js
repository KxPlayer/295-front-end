...
const UploadPage = () => {
    ...
    const handleBuildingUpload = async () => {
      try{
        ...
        const response = await axios.post('http://flask-env.eba-qjhqw3t2.us-east-2.elasticbeanstalk.com/api/building', 
        { 
          "name": buildingName 
        }, 
        { 
    ...
    const handleFileUpload = async (buildingId) => {
        if (file) {
          ...
          try {
            ...
            const response = await axios.post('http://flask-env.eba-qjhqw3t2.us-east-2.elasticbeanstalk.com/api/upload_image', formData, {
                headers: {
